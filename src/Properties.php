...
class Properties extends PropertyCallIterator
{
    ...
    public function addWhereFields($where_key, $where_value)
    {
        $this->where_fields[$where_key] = $where_value;
    }

    /**
     * @param array $where_changes
     */
    public function setWhereChanges($where_changes)
    {
        $this->where_changes = $where_changes;
    }

    /**
     * @param array $where_changes
     */
    public function addWhereChanges($where_key, $where_value)
    {
        $this->where_changes[$where_key] = $where_value;
    }
    ...
    {
        if (in_array($key, array_keys($this->where_fields))) {
            $value  = explode(',', $value);
            $filter = $this->where_fields[$key];
            switch ($filter) {
                case 1:
    ...
    private function merge($operation, $key, $value)
    {
        $string = '';
        $filter = $this->where_fields[$key];
        if (in_array($key, array_keys($this->where_changes))) {
            $key = $this->where_changes[$key];
        }
        if (is_array($filter)) {
